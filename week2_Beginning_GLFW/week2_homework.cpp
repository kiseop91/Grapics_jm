...
void DrawEllipse(float radiusX, float radiusY)
{
	...
	{
		float rad = i * DEG2RAD;
		glVertex2f(cos(rad)*radiusX + 0.1 + circle_center_x,
			sin(rad)*radiusY + 0.1 + circle_center_y);
	}
	glEnd();
	...
	{
		float rad = i * DEG2RAD;
		glVertex2f(cos(rad)*0.025 + 0.1 + circle_center_x,
			sin(rad)*0.05 + 0.125 + circle_center_y);
	}
	glEnd();
	...
	{
		float rad = i * DEG2RAD;
		glVertex2f(cos(rad)*0.1 + circle_center_x,
			sin(rad)*0.1 - 0.1 + circle_center_y);
	}
...
int main(void)
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		glBegin(GL_TRIANGLE_FAN);
		glColor3f(255 / 255.0, 255 / 255.0, 0.0);
