...
int main(void)
{
	...
	glEnable(GL_LIGHT0);  //GL_LIGHT0, 1, 2,....7 광원!
	glEnable(GL_NORMALIZE);  //노멀백터 
	//glShadeModel(GL_SMOOTH); //부드럽게 
	glShadeModel(GL_FLAT);
	glDisable(GL_COLOR_MATERIAL); //

	GLfloat ambientLight[] = { 0.2, 0.2, 0.2, 1.0f };        //엠비언트 라이트!!
	GLfloat lightColor[] = { 15.0f, 12.0f, 10.0f, 1.0f };
	//GLfloat lightPos[] = { 0.0, 0.3, 0.0, 1.0 };
	glm::vec4 lightPos = glm::vec4(0.0f, 0.3f, 0.0f, 1.0f);
	...
	GLfloat materialColor[] = { 1.0f, 1.0f, 0.1f, 1.0f };
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// update object	
		glm::quat rot = glm::angleAxis(3.14f / 4.0f *0.1f, glm::vec3(1.0, 0.0, 0.0)); //돌려버렸
		glm::mat4 m_rot = glm::toMat4(rot);

		std::vector<glm::vec3> &vertices = obj_reader.pos_stack_;
		std::vector<glm::vec3> &normals = obj_reader.nor_stack_;
		...
		for (int vix = 0; vix < vertices.size(); vix++)
		{
			...
			vertices[vix] = m_rot * glm::vec4(vertices[vix], 1.0f);  //정점은 점이지만
			normals[vix] = m_rot * glm::vec4(vertices[vix], 0.0f);   // 노멀백터는 백터다! 마지막인자의차이
		}
