...
public:
	...
	void recalculateNormalVector()
	{
		...
		for (int fix = 0; fix < ix_stack_.size(); fix++)
		{
			...
			//two vectors on the trigangular surgace (plane)
			const vec3 l0 = v1 - v0;
			const vec3 l1 = v2 - v0;

			//normal vector that is normal( 90 degree angle) to l0 and l1;
			// -> this vector is normal to this triangle 
			const vec3 face_normal = cross(l0, l1);
		...
		if (pos_stack_.size() != nor_stack_.size())
		{
			nor_stack_.clear();
		...
		for (int fix = 0; fix < ix_stack_.size(); fix++)
		{
			vec3 &ix_v0 = nor_stack_[ix_stack_[fix].x];
			vec3 &ix_v1 = nor_stack_[ix_stack_[fix].y];
			vec3 &ix_v2 = nor_stack_[ix_stack_[fix].z];


			ix_v0 += face_normals[fix];
			ix_v1 += face_normals[fix];
			ix_v2 += face_normals[fix];
		}
		// normalize vertex normals for smooth average face normals
		for (int vix = 0; vix < nor_stack_.size(); ++vix) {
			//normalize means make the length of the vector to be ONE
			nor_stack_[vix] = normalize(nor_stack_[vix]);
			//nor_stack_[vix] =/ num_of_adjacent_faces;
			//if you use glEnable(GL_NORMALIZE),
			//API normalized normal vector as you did in this function
		}
	}
};
